...
import { resumeAudio, playStress, playDialogue, playThroatHit } from "../audio/gameAudio";
...
export default function Level6Tuning({ onStressChange, onWin }: Props) {
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      resumeAudio();
      if (e.code === "Space") { e.preventDefault(); setCharging(true); }
    };
      ...
      if (e.code === "Space") {
        ...
        setPower(p => {
          ...
          if (pct < 15) {
            ...
            onStressChange(5);
            playStress();
            showMsg("I'm afraid the fork remains silent. A firmer wrist, perhaps.");
          } else if (pct > 80) {
            // too hard
            onStressChange(20);
            playStress();
            showMsg("That was rather vigorous. My ears are still ringing. From a different reason.");
          } else {
            // good strike
  ...
  useEffect(() => {
    ...
    const down = (e: KeyboardEvent) => { pressed[e.key] = true; };
  ...
  useEffect(() => {
    const t = setInterval(() => {
      if (wonRef.current) return;
      if (vibRef.current < 10) { setHoldAt(null); setHoldFrames(0); return; }
  ...
  return (
    <div className="w-full h-full flex items-center justify-center relative select-none"
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 z-20 text-center">
        ...
        <div className="text-xs text-muted-foreground/40 mt-1">Optimal strike: 15–80% charge. Hold at each skull position while vibrating.</div>
      </div>

      {/* Tested positions */}
      <div className="absolute top-20 right-8 z-20">
        <div className="text-xs text-muted-foreground/50 tracking-widest uppercase mb-2">Positions Tested</div>
        {POSITIONS.map(p => (
          <div key={p.id} className="flex items-center gap-2 text-xs mb-1">
            <div className="w-3 h-3 border-2 transition-all"
              style={{ borderColor: tested.has(p.id) ? "hsl(180,60%,55%)" : "hsl(0,0%,22%)", background: tested.has(p.id) ? "hsl(180,60%,15%)" : "transparent" }} />
            <span className={tested.has(p.id) ? "text-primary line-through" : "text-muted-foreground/50"}>{p.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-10">
        ...
        <div>
          <svg viewBox="80 80 210 280" width="300" height="360">
            {/* Head */}
            <ellipse cx="185" cy="210" rx="95" ry="115" fill="hsl(35,55%,62%)" stroke="hsl(35,45%,50%)" strokeWidth="2" />
            {/* Hair */}
            <ellipse cx="185" cy="105" rx="92" ry="42" fill="hsl(25,40%,30%)" />
            {/* Ears */}
            <ellipse cx="90" cy="210" rx="16" ry="22" fill="hsl(35,50%,58%)" stroke="hsl(35,40%,48%)" strokeWidth="1.5" />
            <ellipse cx="280" cy="210" rx="16" ry="22" fill="hsl(35,50%,58%)" stroke="hsl(35,40%,48%)" strokeWidth="1.5" />
            {/* Eyes */}
            ...
            {/* Nose */}
            <path d="M 178 210 Q 185 235 177 248 Q 185 252 193 248 Q 185 235 192 210" fill="hsl(35,40%,55%)" />
            {/* Mouth - slight grimace */}
            <path d="M 160 275 Q 185 270 210 275" fill="none" stroke="hsl(10,30%,40%)" strokeWidth="3" strokeLinecap="round" />
            ...
            {POSITIONS.map(pos => {
              ...
              return (
                <g key={pos.id}>
                  <circle cx={pos.svgX} cy={pos.svgY} r="24"
                    fill={flashPos === pos.id ? "rgba(100,255,200,0.5)" : isTested ? "rgba(100,255,200,0.15)" : isActive ? "rgba(100,255,200,0.2)" : "rgba(255,255,255,0.06)"}
                    stroke={isTested ? "hsl(180,60%,50%)" : isActive ? "hsl(180,60%,55%)" : "rgba(255,255,255,0.2)"}
                    strokeWidth="1.5"
                    strokeDasharray={isTested ? "" : "4 3"} />
                  <text x={pos.svgX} y={pos.svgY + 4} textAnchor="middle"
                    fill={isTested ? "hsl(180,60%,60%)" : "rgba(255,255,255,0.3)"}
                    fontSize="8" fontFamily="monospace">{pos.label}</text>
                  {/* Vibration rings when fork is near */}
                  {isActive && vibration > 10 && [1, 2, 3].map(ring => (
                    <circle key={ring} cx={pos.svgX} cy={pos.svgY}
            ...
            <g transform={`translate(${forkX + shakeX}, ${forkY})`}>
              {/* Handle */}
              <rect x="-3" y="0" width="6" height="50" rx="2" fill="hsl(210,30%,55%)" stroke="hsl(210,25%,45%)" strokeWidth="1" />
              {/* Tines */}
              <rect x="-10" y="-40" width="5" height="46" rx="2" fill="hsl(210,30%,65%)" stroke="hsl(210,25%,50%)" strokeWidth="1" />
              <rect x="5" y="-40" width="5" height="46" rx="2" fill="hsl(210,30%,65%)" stroke="hsl(210,25%,50%)" strokeWidth="1" />
              {/* Tine tips (spread when vibrating) */}
              <circle cx={-7 + (vibration / 100) * -4} cy="-42" r="3" fill="hsl(210,40%,70%)" />
              <circle cx={7 + (vibration / 100) * 4} cy="-42" r="3" fill="hsl(210,40%,70%)" />
              {/* Vibration glow */}
              {vibration > 0 && (
                <ellipse cx="0" cy="-38" rx={8 + vibration / 20} ry="6"
                  fill="none" stroke={`rgba(100,200,255,${vibration / 200})`} strokeWidth="2" />
              )}
            </g>
          </svg>
        </div>
        ...
        <div className="flex flex-col gap-4 w-44">
          {/* Charge meter */}
          <div>
            <div className="text-xs text-muted-foreground/50 tracking-widest uppercase mb-1">Strike Power</div>
            <div className="w-full h-28 bg-secondary overflow-hidden flex flex-col-reverse border border-border/30">
              <div className="w-full transition-none"
                style={{
                  height: `${power}%`,
                  background: power > 80 ? "hsl(0,70%,55%)" : power > 15 ? "hsl(120,60%,45%)" : "hsl(45,80%,55%)",
                }} />
            </div>
            <div className="flex justify-between text-xs text-muted-foreground/30 mt-1">
              <span>SOFT</span>
              <span className={power > 80 ? "text-destructive" : power > 15 ? "text-primary" : "text-yellow-500/60"}>
          ...
          {/* Vibration meter */}
          <div>
            <div className="text-xs text-muted-foreground/50 tracking-widest uppercase mb-1">Vibration</div>
            <div className="w-full h-3 bg-secondary overflow-hidden">
              <div className="h-full transition-none"
                style={{ width: `${vibration}%`, background: vibration > 30 ? "hsl(200,80%,55%)" : "hsl(0,50%,45%)" }} />
            </div>
            <div className="text-xs mt-1" style={{ color: vibration > 30 ? "hsl(200,70%,60%)" : "hsl(0,50%,55%)" }}>
              {vibration > 60 ? "VIBRATING" : vibration > 20 ? "FADING" : "SILENT"}
            </div>
          </div>
          ...
          {holdAt && !tested.has(holdAt) && (
            <div>
              <div className="text-xs text-muted-foreground/50 tracking-widest uppercase mb-1">Hold</div>
              <div className="w-full h-2 bg-secondary overflow-hidden">
                <div className="h-full transition-none"
                  style={{ width: `${holdPct}%`, background: "hsl(180,60%,50%)" }} />
              </div>
            </div>
          )}
        </div>
