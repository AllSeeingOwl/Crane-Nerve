...
export default function Level8Accessory({ onStressChange, onWin }: Props) {
  ...
  useEffect(() => {
    ...
    if (phase === "shrugging") {
      if (bothShrugged) {
        ...
        playStress();
        showMsg(
          DIALOGUE_FAIL[Math.floor(Math.random() * DIALOGUE_FAIL.length)],
        );
        setLeftVel(0);
        setRightVel(0);
        setLeftY(0);
        ...
        showMsg(
          next >= 4
            ? DIALOGUE_SUCCESS[
                Math.floor(Math.random() * DIALOGUE_SUCCESS.length)
              ]
            : DIALOGUE_RESIST[
                Math.floor(Math.random() * DIALOGUE_RESIST.length)
              ],
        );
  ...
  useEffect(() => {
    const t = setInterval(() => {
      if (!showDialogue && !wonRef.current && Math.random() < 0.2) {
        showMsg(
          DIALOGUE_RESIST[Math.floor(Math.random() * DIALOGUE_RESIST.length)],
        );
      }
    }, 9000);
    return () => clearInterval(t);
  ...
  return (
    <div
      ...
      style={{
        background: flashRed
          ? "radial-gradient(ellipse 90% 90% at 50% 50%, rgba(80,0,0,0.82) 0%, rgba(50,0,0,0.22) 100%)"
    ...
    >
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 z-20 text-center">
        ...
        <div className="text-xs text-muted-foreground/40 mt-1">
          Don't let either shoulder rise above threshold. 5 successful
        ...
        <div className="flex gap-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              ...
              style={{
                borderColor:
                  i < resistSuccesses ? "hsl(180,60%,55%)" : "hsl(0,0%,22%)",
                background:
                  i < resistSuccesses ? "hsl(180,60%,15%)" : "transparent",
              }}
            />
          ))}
        </div>
      ...
      <div className="relative">
        <svg viewBox="50 60 280 280" width="380" height="340">
          ...
          <g transform={`translate(0, ${-leftPx})`}>
            ...
            {leftY > 0.5 && (
              <ellipse
                ...
                ry="39"
                fill="none"
                stroke={`rgba(255,80,80,${(leftY - 0.5) * 2 * 0.6})`}
                strokeWidth="2"
              />
            )}
          </g>
          ...
          <g transform={`translate(0, ${-rightPx})`}>
            ...
            {rightY > 0.5 && (
              <ellipse
                ...
                ry="39"
                fill="none"
                stroke={`rgba(255,80,80,${(rightY - 0.5) * 2 * 0.6})`}
                strokeWidth="2"
              />
            )}
          </g>
          ...
          <g
            style={{
              transform: `scale(${handPressureScale})`,
      ...
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-8">
        {[
          ...
          >
            <div className="text-xs text-muted-foreground/40 tracking-widest">
              {side}
            </div>
      ...
      <div className="absolute bottom-16 right-8">
        ...
        <div
          className={`w-16 h-10 border-2 flex items-center justify-center text-xs font-bold transition-all
