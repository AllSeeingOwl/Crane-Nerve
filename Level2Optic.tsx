...
import { resumeAudio, playStress, playDialogue } from "@/lib/gameAudio";
...
export default function Level2Optic({ onStressChange, onWin }: Props) {
  ...
  const [instructions, _setInstructions] = useState(
    "Arrow Keys: guide gaze to chart | Hold steady to read each row",
  );
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 z-20 text-center">
        ...
        <div className="text-xs text-muted-foreground/40 mt-1 tracking-wider">
          Arrow keys — gaze | Control is… aspirational
        </div>
      </div>
        ...
        <div className="flex gap-2">
          {CHART_ROWS.map((_, i) => (
            <div
              ...
              style={{
                borderColor:
                  i < rowsRead ? "hsl(180,60%,55%)" : "hsl(0,0%,22%)",
                background: i < rowsRead ? "hsl(180,60%,15%)" : "transparent",
              }}
            />
          ))}
        </div>
      ...
      <div className="flex items-center gap-16">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <svg viewBox="100 150 170 100" width="280" height="170">
            ...
            />
            {/* Pupil */}
            <circle cx={pupilX} cy={pupilY} r="13" fill="black" />
            {/* Gloss */}
            <circle
              cx={pupilX - 6}
              cy={pupilY - 6}
            ...
            {/* Eyelids */}
            <path
              d={`M ${185 - eyeW / 2} 200 Q 185 ${200 - eyeH * 0.8} ${185 + eyeW / 2} 200`}
              fill="hsl(35,55%,62%)"
              stroke="hsl(35,45%,50%)"
              strokeWidth="2"
            />
            <path
              d={`M ${185 - eyeW / 2} 200 Q 185 ${200 + eyeH * 0.7} ${185 + eyeW / 2} 200`}
              fill="hsl(35,55%,62%)"
              stroke="hsl(35,45%,50%)"
              strokeWidth="2"
            />
            ...
            <div className="w-full h-2 bg-secondary overflow-hidden">
              <div
                ...
                style={{
                  width: `${holdPct}%`,
                  background:
                    holdPct > 60 ? "hsl(180,60%,50%)" : "hsl(45,90%,55%)",
                }}
              />
            </div>
          </div>
        ...
        <div
          ...
          style={{
            background: "white",
            padding: "24px 20px",
            border: "3px solid hsl(210,10%,80%)",
            minWidth: 200,
          }}
        >
          <div
            className="text-xs text-muted-foreground/60 tracking-widest uppercase mb-3"
          ...
          {CHART_ROWS.map((row, i) => (
            <div
              key={i}
              className="text-center font-bold tracking-widest transition-all"
              style={{
      ...
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 w-40 h-32 relative border border-border/30">
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 70% 50%, hsl(180,60%,50%) 0%, transparent 60%)",
          }}
        />
        <div
          className="absolute text-xs text-muted-foreground/30"
        ...
        />
        <div className="absolute bottom-full left-0 right-0 text-center text-xs text-muted-foreground/30 tracking-widest mb-1">
          GAZE MAP
        </div>
      </div>
