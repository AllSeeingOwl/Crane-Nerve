...
export default function Level4Trigeminal({ onStressChange, onWin }: Props) {
  ...
  const [lagQueue, setLagQueue] = useState<
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent, toolOverride?: "soft" | "sharp") => {
      resumeAudio();
      if (won) return;
    ...
    >
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 text-center z-20">
        ...
        <div className="text-xs text-muted-foreground/50 mt-1">
          Touch all 4 zones with soft cotton only
        </div>
      </div>
      ...
      <div className="absolute top-24 right-8 flex flex-col gap-2 z-20">
        {ZONES.map((z) => (
          <div key={z.id} className="flex items-center gap-2 text-xs">
            <div
              className={`w-3 h-3 border ${touched.has(z.id) ? "bg-primary border-primary" : "border-border"}`}
            />
            <span
              className={
                touched.has(z.id) ? "text-primary" : "text-muted-foreground/50"
              }
            >
              {z.label}
            </span>
      ...
      <div className="relative" style={{ width: 370, height: 370 }}>
        <svg viewBox="0 0 370 370" width="370" height="370">
          ...
          <rect
            ...
            strokeWidth="1.5"
            strokeDasharray={touched.has("forehead") ? "" : "4 4"}
          />
          {/* Eyes */}
          ...
          {/* Nose */}
          <path
            d="M 180 170 Q 185 200 175 215 Q 185 220 195 215 Q 185 200 190 170"
            fill="hsl(35,40%,55%)"
            stroke="hsl(35,35%,48%)"
            strokeWidth="1"
          />
          ...
          <rect
            ...
            strokeWidth="1.5"
            strokeDasharray={touched.has("left-cheek") ? "" : "4 4"}
          />
          {/* Right cheek zone */}
          <rect
            x="220"
            ...
            strokeWidth="1.5"
            strokeDasharray={touched.has("right-cheek") ? "" : "4 4"}
          />
          {/* Mouth */}
          <path
            d="M 155 255 Q 185 275 215 255"
            fill="none"
            stroke="hsl(10,30%,40%)"
            strokeWidth="3"
            strokeLinecap="round"
          ...
          <rect
            ...
            stroke={
              touched.has("chin") ? "hsl(180,60%,50%)" : "rgba(255,255,255,0.1)"
            }
            strokeWidth="1.5"
            strokeDasharray={touched.has("chin") ? "" : "4 4"}
          />
          {/* Zone labels */}
          <text
            x="185"
      ...
      {/* Real cursor ghost */}
      <div
        className="absolute pointer-events-none z-30 opacity-25"
        style={{
          left: realMouse.x,
          top: realMouse.y,
          width: 6,
      ...
      {showDialogue && (
        <div
          className="absolute top-40 right-12 z-30 max-w-xs"
          style={{
      ...
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-6">
        <div
          className={`flex items-center gap-2 text-xs ${currentTool === "soft" ? "text-primary" : "text-muted-foreground/40"}`}
        >
          <div className="w-4 h-4 rounded-full border-2 border-current" />
          LEFT CLICK — SOFT
        </div>
        <div
          className={`flex items-center gap-2 text-xs ${currentTool === "sharp" ? "text-destructive" : "text-muted-foreground/40"}`}
        >
          <div
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-xs text-muted-foreground/30 tracking-widest">
        NOTE: The lagged cursor is the real one. The ghost is just for
        reference.
      </div>
    </div>
  );
