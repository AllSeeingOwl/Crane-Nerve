...
export default function Level7GagReflex({
  onStressChange,
  onWin,
  onLose,
}: Props) {
  ...
  const [_instructions, _setInstructions] = useState(
    "Navigate the tongue depressor to the back of the throat. W/A/S/D to move, mouse to angle.",
  );
  ...
  useEffect(() => {
    const scheduleSnap = () => {
      ...
      return setTimeout(() => {
        ...
        showMsg(
          DIALOGUE_NEAR[Math.floor(Math.random() * DIALOGUE_NEAR.length)],
        );
        playMouthWarning();
        // Close mouth quickly
        let open = 1;
        const close = setInterval(() => {
          ...
          if (open <= 0) {
            ...
            if (d.y > 0.3 && d.x > 0.2 && d.x < 0.8) {
              ...
              showMsg(
                DIALOGUE_BITE[Math.floor(Math.random() * DIALOGUE_BITE.length)],
              );
    ...
    >
        ...
        <div className="flex gap-2">
          {[0, 1, 2].map((i) => (
            <div
              ...
              style={{
                borderColor: i < touches ? "hsl(180,60%,55%)" : "hsl(0,0%,22%)",
                background: i < touches ? "hsl(180,60%,15%)" : "transparent",
              }}
            />
          ))}
        </div>
      ...
      <div className="relative">
        <svg viewBox="0 80 370 360" width="370" height="340">
          ...
          {/* Tongue depressor */}
          <g
            transform={`translate(${depX}, ${depY}) rotate(${depressorAngle})`}
          >
            {/* Handle */}
            <rect
              x="-3"
        ...
        <div className="w-3 h-24 bg-secondary overflow-hidden flex flex-col-reverse">
          <div
            ...
            style={{
              ...
              background:
                mouthOpen > 0.5 ? "hsl(180,60%,50%)" : "hsl(0,70%,55%)",
            }}
          />
        </div>
      </div>
      ...
      {biteWarning && (
        <div className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none">
          <div
            className="text-4xl font-bold text-destructive tracking-widest"
