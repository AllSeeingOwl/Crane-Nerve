...
export default function Level3EyeMovement({ onStressChange, onWin }: Props) {
  ...
  useEffect(() => {
    const tick = setInterval(() => {
      ...
      const lx = lightRef.current.x;
      const ly = lightRef.current.y;
      const _ex = eyeRef.current.x;
      const _ey = eyeRef.current.y;
      const LERP = 0.04;
  ...
  const _targetSvgX = currentTarget ? SVG_CX + currentTarget.x : SVG_CX;
  const _targetSvgY = currentTarget ? SVG_CY + currentTarget.y : SVG_CY;
  ...
  const clampedEyeX =
    mag > 0.65 ? SVG_CX + (eyeDx / mag) * sclRx * 0.65 : rawEyeX;
  const clampedEyeY =
    mag > 0.65 ? SVG_CY + (eyeDy / mag) * sclRy * 0.65 : rawEyeY;
    ...
    >
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 z-20 text-center">
        ...
        <div className="text-xs text-muted-foreground/40 mt-1">
          The light overshoots. The eye has its own plans.
        </div>
      </div>
        ...
        <div className="flex flex-col gap-1">
          {H_POSITIONS.map((p, i) => (
            <div key={i} className="flex items-center gap-2 text-xs">
              <div
                className={`w-2.5 h-2.5 border ${i < step ? "bg-primary border-primary" : i === step ? "border-primary animate-pulse" : "border-border"}`}
              />
              <span
                className={
                  i < step
      ...
      <div>
        <svg viewBox="80 150 210 120" width="420" height="240">
          ...
          {/* Eyelids */}
          <path
            d={`M ${SVG_CX - sclRx} ${SVG_CY} Q ${SVG_CX} ${SVG_CY - sclRy * 1.4} ${SVG_CX + sclRx} ${SVG_CY}`}
            fill="hsl(35,55%,62%)"
            stroke="hsl(35,45%,50%)"
            strokeWidth="2"
          />
          <path
            d={`M ${SVG_CX - sclRx} ${SVG_CY} Q ${SVG_CX} ${SVG_CY + sclRy * 1.3} ${SVG_CX + sclRx} ${SVG_CY}`}
            fill="hsl(35,55%,62%)"
            stroke="hsl(35,45%,50%)"
            strokeWidth="2"
          />
          ...
          {[-60, -30, 0, 30, 60].map((angle) => {
            ...
            return (
              <line
                ...
                x1={bx}
                y1={by}
                x2={bx + Math.cos(rad - Math.PI / 2) * 8}
                y2={by - 8}
                stroke="hsl(25,40%,25%)"
                strokeWidth="1.5"
                strokeLinecap="round"
      ...
      <div className="absolute bottom-20 left-1/2 -translate-x-1/2 w-56">
        ...
        <div className="w-full h-2 bg-secondary overflow-hidden">
          <div
            ...
            style={{
              width: `${holdPct}%`,
              background: holdPct > 60 ? "hsl(180,60%,50%)" : "hsl(45,90%,55%)",
            }}
          />
        </div>
      </div>
